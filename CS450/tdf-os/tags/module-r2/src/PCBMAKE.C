...
int pcreate(char *name, PCBDLL **PRIORITY_QUEUES){
  PCB *temp;
  ...
  if( name == NULL ) { // No name argument
    ...
    temp = PCB_setup(name, priority, pclass, PRIORITY_QUEUES);
...
int pdelete(char *name, PCBDLL **PRIORITY_QUEUES){
  ...
  PCB_DELETE_ERR = PCB_remove(PCB_find(name, PRIORITY_QUEUES), PRIORITY_QUEUES);
